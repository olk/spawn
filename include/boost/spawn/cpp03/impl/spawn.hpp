...
public:
    ...
    spawn_context( std::allocator_arg_t, StackAlloc const& salloc, Fn && fn) :
        ctx_(
            std::allocator_arg,
    ...
    void resume() {
        ...
        if ( eptr_) {
            boost::rethrow_exception( std::move( eptr_) );
        }
    }
};
...
struct spawn_helper {
    void operator()() {
        ...
        if ( callee_->eptr_) {
            boost::rethrow_exception( std::move( callee_->eptr_) );
        }
    }
