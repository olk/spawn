...
private:
    void echo(boost::spawn::yield_context yield) {
        try {
            char data[128];
            for (;;) {
    ...
    void timeout(boost::spawn::yield_context yield) {
...
void do_accept(boost::asio::io_context& io_context,
        unsigned short port, boost::spawn::yield_context yield) {
