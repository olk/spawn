...
struct yield_result : boost::asio::async_result<boost::spawn::yield_context, Sig> {};
...
struct counting_handler {
    ...
    template< typename T >
    void operator()( boost::spawn::basic_yield_context< T >) {
        ++count;
    }
};
...
void spawnFunction() {
    ...
    int called = 0;
    boost::spawn_fiber( counting_handler{ called } );
...
void spawnBoundFunction() {
    boost::asio::io_context ioc;
    int called = 0;
    boost::spawn_fiber( bind_executor( ioc.get_executor(), counting_handler{ called } ) );
...
struct spawn_counting_handler {
    ...
    template< typename T >
    void operator()( boost::spawn::basic_yield_context< T > y) {
        boost::spawn_fiber( y, counting_handler{ count } );
        ++count;
    }
};
...
void spawnExecutor() {
    boost::asio::io_context ioc;
    int called = 0;
    boost::spawn_fiber( ioc.get_executor(), counting_handler{ called } );
    BOOST_CHECK_EQUAL(1, ioc.run());
    BOOST_CHECK(ioc.stopped());
    BOOST_CHECK_EQUAL(1, called);
}
...
void spawnExecutionContext() {
    boost::asio::io_context ioc;
    int called = 0;
    boost::spawn_fiber( ioc, counting_handler{ called } );
    BOOST_CHECK_EQUAL(1, ioc.run());
    BOOST_CHECK(ioc.stopped());
    BOOST_CHECK_EQUAL(1, called);
}
...
struct spawn_wait_handler {
    ...
    template< typename T >
    void operator()( boost::spawn::basic_yield_context< T > yield) {
...
struct single_tuple_handler {
    ...
    void operator()( boost::spawn::yield_context y) {
        using Signature = void(std::tuple<int, std::string>);
        boost::asio::async_completion< boost::spawn::yield_context, Signature > init{ y };
        post(
...
void returnSingleTuple() {
    boost::asio::io_context ioc;
    std::tuple< int, std::string > result;
    boost::spawn_fiber( ioc, single_tuple_handler{ result } );
...
void returnMultiple2() {
    boost::asio::io_context ioc;
    std::tuple< int, std::string > result;
    boost::spawn_fiber( ioc, multiple2_handler{result});
    BOOST_CHECK_EQUAL(2, ioc.poll() );
    BOOST_CHECK(std::make_tuple(42, std::string{"test"}) == result);
}
...
void returnMultiple2MoveOnly() {
    boost::asio::io_context ioc;
    std::tuple< int, std::unique_ptr< int > > result;
    boost::spawn_fiber( ioc, multiple2_with_moveonly_handler{ result } );
...
void returnMultiple3() {
    boost::asio::io_context ioc;
    std::tuple< int, std::string, double > result;
    boost::spawn_fiber( ioc, multiple3_handler{result});
    BOOST_CHECK_EQUAL(2, ioc.poll() );
...
void returnNonDefaultConstructible() {
    boost::asio::io_context ioc;
    boost::optional< non_default_constructible > result;
    boost::spawn_fiber( ioc, non_default_constructible_handler{ result } );
    BOOST_CHECK_EQUAL(2, ioc.poll() );
    BOOST_CHECK(result);
}
...
void returnMultipleNonDefaultConstructible() {
    boost::asio::io_context ioc;
    boost::optional< std::tuple< int, non_default_constructible > > result;
    boost::spawn_fiber( ioc, multiple_non_default_constructible_handler{ result } );
    BOOST_CHECK_EQUAL(2, ioc.poll() );
    BOOST_CHECK(result);
}
...
void spawnThrowInHelper() {
    boost::asio::io_context ioc;
    boost::spawn_fiber( ioc, throwing_handler{} );
...
void spawnThrowAfterYield() {
    boost::asio::io_context ioc;
    boost::spawn_fiber( ioc, yield_throwing_handler{} );
...
struct nested_throwing_handler {
    template< typename T >
    void operator()( boost::spawn::basic_yield_context< T > y) {
        boost::spawn_fiber( y, throwing_handler{} );
    }
};

void spawnThrowInNestedHelper() {
    boost::asio::io_context ioc;
    boost::spawn_fiber( ioc, nested_throwing_handler{} );
...
struct yield_nested_throwing_handler {
    ...
    void operator()( boost::spawn::basic_yield_context< T > y) {
        ...
        boost::spawn_fiber( y, yield_throwing_handler{} );
    }
};

void spawnThrowAfterNestedYield() {
    boost::asio::io_context ioc;
    boost::spawn_fiber( ioc, yield_nested_throwing_handler{} );
...
struct yield_throw_after_nested_handler {
    ...
    void operator()( boost::spawn::basic_yield_context< T > y) {
        async_yield( y); // suspend and resume before spawning
        boost::spawn_fiber( y, yield_handler{} );
        throw std::runtime_error{ "" };
    }
};
...
void spawnThrowAfterNestedSpawn() {
    boost::asio::io_context ioc;
    boost::spawn_fiber( ioc, yield_throw_after_nested_handler() );
...
boost::unit_test::test_suite * init_unit_test_suite( int, char* []) {
    boost::unit_test::test_suite * test =
        BOOST_TEST_SUITE("Boost.Spawn: spawn test suite");
